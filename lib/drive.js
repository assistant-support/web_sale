...
export async function uploadBufferToDrive(bufferOrOptions, fileName, folderId, mimeType = 'application/octet-stream') {
    try {
        ...
        // Cấp quyền xem công khai cho file để có thể truy cập qua URL
        if (response.data?.id) {
            try {
                ...
                console.log(`✅ Đã cấp quyền xem công khai cho file ${response.data.id}`);
            } catch (permError) {
                console.error(`⚠️ Không thể cấp quyền cho file ${response.data.id}:`, permError?.message);
                // Không throw lỗi để quá trình upload chính vẫn thành công
            }
        }
...
// Cấp quyền xem công khai cho file đã tồn tại trên Google Drive
export async function setPublicPermission(fileId) {
    ...
    try {
        ...
        console.error(`⚠️ Không thể cấp quyền cho file ${fileId}:`, error?.message);
