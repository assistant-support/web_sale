...
export async function getPagesFromAPI() {
    try {
        ...
        if (response.data?.success && response.data?.categorized?.activated) {
            // Transform the API data into the format our app uses
            return response.data.categorized.activated.map(page => ({
                id: page.id,
                name: page.name,
                // The page access token is found inside the settings object
            ...
            })).filter(page => page.accessToken); // Only include pages that have an access token
        }
